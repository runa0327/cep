...
public class PmDeptUserImportController {
    ...
    public Map<String, Object> importData(MultipartFile file, String projectId) {
        Map<String, Object> result = new HashMap<>();
        List<String> res = new ArrayList<>();
        List<PmDeptUserModel> dataList = EasyExcelUtil.read(file.getInputStream(), PmDeptUserModel.class);
        Map<String, List<PmDeptUserModel>> mapData = dataList.stream().collect(Collectors.groupingBy(PmDeptUserModel::getDeptName));
            ...
            if (optional.isPresent()) {
                ...
                for (PmDeptUserModel pmDeptUserModel : userModelList) {
                    ...
                    if (userOptional.isPresent()) {
                        ...
                        res.add(pmDeptUserModel.getUserName());
                    }
                }
        ...
        if (CollectionUtils.isEmpty(res)) {
            ...
            result.put("code", 500);
            result.put("message", "部门名称或用户名称为:" + String.join(",", res) + "不存在，未导入！");
            return result;
        }
    }
}
