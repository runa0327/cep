...
public class ProFileJob {
    ...
    public void invokeRefreshProFile() {
        ...
        dataList.forEach(subList -> {
            ...
            // List<String> ids = subList.stream().map(m -> JdbcMapUtil.getString(m, "ID")).collect(Collectors.toList());
            // List<Integer> vers = subList.stream().map(m -> Integer.parseInt(JdbcMapUtil.getString(m, "VER"))).collect(Collectors.toList());
            taskExecutor.execute(() -> {
                InvokeActParam invokeActParam = new InvokeActParam();
                invokeActParam.sevId = "0099799190825090826";
                invokeActParam.actId = "0099902212142088375";
                invokeActParam.isPreChk = false;
                // invokeActParam.idList = ids;
                // invokeActParam.verList = vers;
                invokeActParam.valueMapList = subList.stream().map(item -> {
                    HashMap<String, Object> hashMap = new HashMap<>(2);
                    hashMap.put(SharedConstants.Cols.ID, JdbcMapUtil.getString(item, "ID"));
                    hashMap.put(SharedConstants.Cols.VER, JdbcMapUtil.getInt(item, "VER"));
                    return hashMap;
