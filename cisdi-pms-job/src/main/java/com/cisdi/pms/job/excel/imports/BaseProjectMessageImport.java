        ...
        try {
            ...
            if (!CollectionUtils.isEmpty(keYanList)){
                for (PmPrjInvest1Model tmp : keYanList) {
                    String insertSql = "insert into PM_PRJ_INVEST1 (id,ver,status,PM_PRJ_ID,REPLY_NO_WR,PRJ_TOTAL_INVEST,PROJECT_AMT,CONSTRUCT_AMT,EQUIP_AMT,PROJECT_OTHER_AMT,LAND_AMT,PREPARE_AMT," +
                            "CONSTRUCT_PERIOD_INTEREST) values ((select uuid_short()),'99','AP',?,?,?,?,?,?,?,?,?,?)";
            ...
            if (!CollectionUtils.isEmpty(chuGaiList)){
                for (PmPrjInvest2Model tmp : chuGaiList) {
                    String insertSql = "insert into PM_PRJ_INVEST2 (id,ver,status,PM_PRJ_ID,REPLY_NO_WR,PRJ_TOTAL_INVEST,PROJECT_AMT,CONSTRUCT_AMT,EQUIP_AMT,PROJECT_OTHER_AMT,LAND_AMT,PREPARE_AMT," +
                            "CONSTRUCT_PERIOD_INTEREST) values ((select uuid_short()),'99','AP',?,?,?,?,?,?,?,?,?,?)";
                    int update1 = jdbcTemplate.update(insertSql,tmp.getPM_PRJ_ID(),tmp.getREPLY_NO_WR(),tmp.getPRJ_TOTAL_INVEST(),tmp.getPROJECT_AMT(),tmp.getCONSTRUCT_AMT(),tmp.getEQUIP_AMT(),
                            tmp.getPROJECT_OTHER_AMT(),tmp.getLAND_AMT(),tmp.getPREPARE_AMT(),tmp.getCONSTRUCT_PERIOD_INTEREST());
                    num3 = num3 + update1;
            ...
            msg = "成功执行。项目表更新： " + num + " 条数据，可研表更新： " + num2 + " 条数据，初概表更新： " + num3 + " 条数据";
            errBuffer.append("\n").append(msg);
        } catch (Exception e){
            e.printStackTrace();
        }
