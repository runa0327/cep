 ...
 * @title ProjectPostImportController
 * @package com.cisdi.pms.job.excel.imports
 * @description 项目岗位导入
 * @date 2023/2/13
 */
...
public class ProjectPostImportController extends BaseController {
    ...
    public Map<String, Object> importData(MultipartFile file, HttpServletResponse response) {
        ...
        List<ProjectPostModel> list = EasyExcelUtil.read(file.getInputStream(), ProjectPostModel.class);
        //去除空行
        List<ProjectPostModel> postModelList = list.stream().filter(p -> !ReflectUtil.isObjectNull(p)).collect(Collectors.toList());
        //如果有不能处理的字段，响应提示
