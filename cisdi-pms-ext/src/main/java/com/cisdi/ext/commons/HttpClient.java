...
public class HttpClient {
     ...
     * @return 接口地址
     */
    public static String getUrl(String code,MyJdbcTemplate myJdbcTemplate) {
        String url = "";
        String httpSql = "select HOST_ADDR from BASE_THIRD_INTERFACE where code = ? and SYS_TRUE = 1";
        List<Map<String,Object>> listUrl = myJdbcTemplate.queryForList(httpSql,code);
        if (!CollectionUtils.isEmpty(listUrl)) {
            url = listUrl.get(0).get("HOST_ADDR").toString();
        }
        return url;
    }
}
