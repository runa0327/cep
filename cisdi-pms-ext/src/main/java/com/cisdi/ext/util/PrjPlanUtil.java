...
public class PrjPlanUtil {
    ...
    public static void refreshProPlanTime(String projectId,Date date){
        ...
        List<Map<String,Object>> mapZero = jdbcTemplate.queryForList("select PRJ_REPLY_DATE from PM_PRJ where id = ? ",projectId);
        ...
        String sql = "SELECT b.id,b.PLAN_START_DATE,b.PLAN_COMPL_DATE,b.PLAN_TOTAL_DAYS,b.NAME,b.START_DAY,ifnull(b.PM_PRO_PLAN_NODE_PID,'0') as PM_PRO_PLAN_NODE_PID " +
                ...
                "WHERE c.id = ?";
        List<Map<String,Object>> map = jdbcTemplate.queryForList(sql,projectId);
        ...
        jdbcTemplate.update("update PM_PRO_PLAN set PLAN_COMPL_DATE = ?,PLAN_TOTAL_DAYS = ?,PLAN_START_DATE=?,START_DAY=? where PM_PRJ_ID = ?",planCom,mainDays,planStart,start,projectId);
