    ...
    public void pmStartModify() {
        ...
        if (CollectionUtils.isEmpty(list)) {
            ...
            Crud.from("PM_PRJ").where().eq("ID", projectId).update().set("NAME", input.name).set("PM_CODE", prjCode)
                    ...
                    .set("PRJ_SITUATION", input.description).set("PM_SEQ", seq).set("BUILDER_UNIT", input.unit).set("ESTIMATED_TOTAL_INVEST", input.invest.multiply(new BigDecimal(10000))).exec();
        } else {
