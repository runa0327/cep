...
//        List<Map<String, Object>> prjMaps = myJdbcTemplate.queryForList("SELECT id prjId,name prjName FROM pm_prj WHERE status = 'AP' and name = '测试2'");//测试
        ...
        List<Map<String, Object>> prjReqMaps = myJdbcTemplate.queryForList("SELECT '1631460206540161024' priorityId, pj.id prjId, pr.PRJ_NAME " +
                "prjName, pr.EQUIPMENT_COST, pr.PREPARE_AMT, pr.LAND_AMT, pr.PROJECT_OTHER_AMT, pr.EQUIP_AMT, pr.PROJECT_AMT, pr.CONSTRUCT_AMT, " +
                "pr.PRJ_TOTAL_INVEST,pr.PRJ_REPLY_NO replyNo,pr.PRJ_REPLY_DATE replyDate " +
                "FROM pm_prj_req pr left join pm_prj pj on pj.name = pr.PRJ_NAME\n" +
        ...
        List<Map<String, Object>> feasibilityMaps = myJdbcTemplate.queryForList("SELECT '1631460270226472960' priorityId, i.EQUIPMENT_COST, i" +
                ".PREPARE_AMT, i.LAND_AMT, i.PROJECT_OTHER_AMT, i.EQUIP_AMT, i.CONSTRUCT_AMT, i.PROJECT_AMT, i.PRJ_TOTAL_INVEST," +
                "i.CONSTRUCT_PERIOD_INTEREST, i.PM_PRJ_ID prjId, i.REPLY_NO_WR replyNo, i.REPLY_ACTUAL_DATE replyDate" +
                " FROM pm_prj_invest1 i \n" +
                "left join (select PM_PRJ_ID,max(CRT_DT) maxCrtDt from pm_prj_invest1 where status = 'AP' group by PM_PRJ_ID) temp on temp.PM_PRJ_ID = i.PM_PRJ_ID \n" +
        ...
        List<Map<String, Object>> estimateMaps = myJdbcTemplate.queryForList("SELECT '1631460343433854976' priorityId, i.EQUIPMENT_COST, i" +
                ...
                "left join (select PM_PRJ_ID,max(CRT_DT) maxCrtDt from pm_prj_invest2 where status = 'AP' group by PM_PRJ_ID) temp on temp.PM_PRJ_ID = i.PM_PRJ_ID \n" +
                "where i.status = 'AP' and i.PM_PRJ_ID is not null and i.CRT_DT = temp.maxCrtDt and (i.EQUIPMENT_COST is not null or i.PREPARE_AMT " +
                "is not null or i.LAND_AMT is not null or i.PROJECT_OTHER_AMT is not null or i.EQUIP_AMT is not null or i.PROJECT_AMT is not null " +
                "or i.CONSTRUCT_AMT is not null or i.PRJ_TOTAL_INVEST is not null or i.CONSTRUCT_PERIOD_INTEREST is not null)");
        //财评表
        List<Map<String, Object>> evaluationMaps = myJdbcTemplate.queryForList("SELECT '1631460383644647424' priorityId, i.PM_PRJ_ID prjId, i" +
                ".PRJ_TOTAL_INVEST, i.PREPARE_AMT, i.LAND_AMT, i.PROJECT_OTHER_AMT, i.EQUIP_AMT, i.CONSTRUCT_AMT, i.PROJECT_AMT," +
                "i.CONSTRUCT_PERIOD_INTEREST,i.REPLY_NO_WR replyNo, i.FINANCIAL_ACTUAL_DATE replyDate " +
                "FROM pm_prj_invest3 i \n" +
                "left join (select PM_PRJ_ID,max(CRT_DT) maxCrtDt from pm_prj_invest3 where status = 'AP' group by PM_PRJ_ID) temp on temp.PM_PRJ_ID = i.PM_PRJ_ID\n" +
    ...
    private void doUpdatePrjInvest(Map<String,Object> investMap){
        Crud.from("pm_prj").where().eq("ID",investMap.get("prjId")).update()
                ...
                .set("REPLY_DATE",investMap.get("replyDate"))
                .set("REPLY_NO",investMap.get("replyNo"))
                .exec();
    }
