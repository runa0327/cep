...
//        List<Map<String, Object>> prjMaps = myJdbcTemplate.queryForList("SELECT id prjId,name prjName FROM pm_prj WHERE status = 'AP' and name = '测试2'");//测试
        //立项表
        List<Map<String, Object>> prjReqMaps = myJdbcTemplate.queryForList("SELECT '1631460206540161024' priorityId, pj.id prjId, pr.PRJ_NAME prjName, pr.EQUIPMENT_COST, pr" +
                ".PREPARE_AMT, pr.LAND_AMT, pr.PROJECT_OTHER_AMT, pr.EQUIP_AMT, pr.PROJECT_AMT, pr.CONSTRUCT_AMT, pr.PRJ_TOTAL_INVEST FROM " +
                ...
                "where pr.status = 'AP' and pr.CRT_DT = temp.maxCrtDt and pj.id is not null");
        //可研表
        List<Map<String, Object>> feasibilityMaps = myJdbcTemplate.queryForList("SELECT '1631460270226472960' priorityId, i.EQUIPMENT_COST, i.PREPARE_AMT, i.LAND_AMT, i" +
                ".PROJECT_OTHER_AMT, i.EQUIP_AMT, i.CONSTRUCT_AMT, i.PROJECT_AMT, i.PRJ_TOTAL_INVEST, i.PM_PRJ_ID prjId FROM pm_prj_invest1 i \n" +
                "left join (select PM_PRJ_ID,max(CRT_DT) maxCrtDt from pm_prj_invest1 group by PM_PRJ_ID) temp on temp.PM_PRJ_ID = i.PM_PRJ_ID \n" +
                "where i.status = 'AP' and i.PM_PRJ_ID is not null and i.CRT_DT = temp.maxCrtDt");
        //概算表
        List<Map<String, Object>> estimateMaps = myJdbcTemplate.queryForList("SELECT '1631460343433854976' priorityId, i.EQUIPMENT_COST, i.PREPARE_AMT, i.LAND_AMT, i" +
                ".PROJECT_OTHER_AMT, i.EQUIP_AMT, i.CONSTRUCT_AMT, i.PROJECT_AMT, i.PRJ_TOTAL_INVEST, i.PM_PRJ_ID prjId FROM pm_prj_invest2 i \n" +
                "left join (select PM_PRJ_ID,max(CRT_DT) maxCrtDt from pm_prj_invest2 group by PM_PRJ_ID) temp on temp.PM_PRJ_ID = i.PM_PRJ_ID \n" +
                "where i.status = 'AP' and i.PM_PRJ_ID is not null and i.CRT_DT = temp.maxCrtDt");
        //财评表
        List<Map<String, Object>> evaluationMaps = myJdbcTemplate.queryForList("SELECT '1631460383644647424' priorityId, i.PM_PRJ_ID prjId, i.PRJ_TOTAL_INVEST, i.PREPARE_AMT, i" +
                ".LAND_AMT, i.PROJECT_OTHER_AMT, i.EQUIP_AMT, i.CONSTRUCT_AMT, i.PROJECT_AMT FROM pm_prj_invest3 i " +
