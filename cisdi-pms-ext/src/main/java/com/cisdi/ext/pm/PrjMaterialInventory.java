...
//        String sql = "select id from pm_prj where status = 'AP' and PROJECT_SOURCE_TYPE_ID = '0099952822476441374' and IZ_FORMAL_PRJ = 1 and PROJECT_CLASSIFICATION_ID = '1704686664114929664' ";
        ...
        sql += " order by PM_SEQ DESC  limit :start,:pageSize";
        ...
        if (!CollectionUtils.isEmpty(prjIdMaps)) {
            ...
            originList = myNamedParameterJdbcTemplate.queryForList(orgSql, params);
            //封装
...
//            }
    ...
    public void materialList() {
        MyJdbcTemplate myJdbcTemplate = ExtJarHelper.myJdbcTemplate.get();
        List<Map<String, Object>> list = myJdbcTemplate.queryForList("select v.id,v.name from gr_set_value v left join gr_set s on s.id = v.GR_SET_ID " +
                "where s.code = 'file_master_list_type' and v.status = 'AP'");
