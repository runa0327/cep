...
//        sb.append(" group by ft.`NAME`");
     ...
     * 统计表（包含项目）
     */
    public void fundListIncludePrj() {
        Map<String, Object> map = ExtJarHelper.extApiParamMap.get();// 输入参数的map。
        String json = JsonUtil.toJson(map);
        RequestParam param = JsonUtil.fromJson(json, RequestParam.class);
        Integer pageIndex = param.pageIndex;
        ...
        sb.append("select fi.id,ft.name as categoryName,ft.id categoryNameId,ft1.name as categoryNameSecond,ft1.id categoryNameSecondId,fi" +
                ...
                "left join (select sum(REACH_AMOUNT) sumAmt,FUND_SOURCE_TEXT from fund_reach group by FUND_SOURCE_TEXT) temp on temp\n" +
                ".FUND_SOURCE_TEXT = fi.FUND_SOURCE_TEXT \n" +
                "left join (select sum(REACH_AMOUNT) sumZqAmt,FUND_SOURCE_TEXT from fund_reach where FUND_REACH_CATEGORY = '99952822476371281' " +
                "group by \n" +
                "FUND_SOURCE_TEXT) temp2 on temp2.FUND_SOURCE_TEXT = fi.FUND_SOURCE_TEXT \n" +
     ...
     * @return
     */
    private DataListObjectIncludePrj convertDataIncludePrj(Map<String, Object> data) {
     ...
     * 列表返回结果类
     */
    public static class DataListObjectIncludePrj {
        //资金大类
        public String categoryName;
        public String categoryNameId;
        //资金大类二级
     ...
     * 返回值包装类
     */
    public static class OutSideIncludePrj {
        public Integer total;
        public List<DataListObjectIncludePrj> list;
    }
}
