...
public class PmParty {
     ...
     * @return 获取到的对象，若无则为null。
     */
    public static PmParty selectById(String id) {
     ...
     * @return 获取到的对象列表，若无则为null。建议使用SharedUtil.isEmptyList(list)方法判断有无。
     */
    public static List<PmParty> selectByIds(List<String> ids) {
     ...
     * @return 获取到的对象列表，若无则为null。建议使用SharedUtil.isEmptyList(list)方法判断有无。
     */
    public static List<PmParty> selectByWhere(Where where) {
     ...
     * @return 获取到的对象。
     */
    public static PmParty selectOneByWhere(Where where, List<String> includeCols, List<String> excludeCols) {
        List<PmParty> objList = modelHelper.selectByWhere(where, includeCols, excludeCols);
        if (objList != null && objList.size() > 1) {
            throw new BaseException("调用PmParty.selectOneByWhere方法不能返回" + objList.size() + "条记录（只能返回0条或1条）！");
        }
     ...
     * @return 获取到的对象。
     */
    public static PmParty selectOneByWhere(Where where) {
     ...
     * @param toModel   到模型。
     */
    public static void copyCols(PmParty fromModel, PmParty toModel) {
