...
public class TkTaskDtl {
    ...
    public TkTaskDtl setTaskImplClass(String taskImplClass) {
        if (this.taskImplClass == null && taskImplClass == null) {
            // 均为null，不做处理。
        } else if (this.taskImplClass != null && taskImplClass != null) {
            // 均非null，判定不等，再做处理：
            if (this.taskImplClass.compareTo(taskImplClass) != 0) {
                this.taskImplClass = taskImplClass;
                if (!this.toUpdateCols.contains("TASK_IMPL_CLASS")) {
                    this.toUpdateCols.add("TASK_IMPL_CLASS");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
    ...
    public TkTaskDtl setTaskArgs(String taskArgs) {
        if (this.taskArgs == null && taskArgs == null) {
            // 均为null，不做处理。
        } else if (this.taskArgs != null && taskArgs != null) {
            // 均非null，判定不等，再做处理：
            if (this.taskArgs.compareTo(taskArgs) != 0) {
                this.taskArgs = taskArgs;
                if (!this.toUpdateCols.contains("TASK_ARGS")) {
                    this.toUpdateCols.add("TASK_ARGS");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
     ...
     * 设置：序号。
     */
    public TkTaskDtl setSeqNo(BigDecimal seqNo) {
    ...
    public TkTaskDtl setBreakOnError(Boolean breakOnError) {
        if (this.breakOnError == null && breakOnError == null) {
            // 均为null，不做处理。
        } else if (this.breakOnError != null && breakOnError != null) {
            // 均非null，判定不等，再做处理：
            if (this.breakOnError.compareTo(breakOnError) != 0) {
                this.breakOnError = breakOnError;
                if (!this.toUpdateCols.contains("BREAK_ON_ERROR")) {
                    this.toUpdateCols.add("BREAK_ON_ERROR");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
    ...
    public static TkTaskDtl newData() {
        TkTaskDtl obj = modelHelper.newData();
        return obj;
    }
    ...
    public static TkTaskDtl insertData() {
        TkTaskDtl obj = modelHelper.insertData();
        return obj;
    }
    ...
    public static TkTaskDtl selectById(String id, List<String> includeCols, List<String> excludeCols) {
        TkTaskDtl obj = modelHelper.selectById(id, includeCols, excludeCols);
        return obj;
    }
    ...
    public static List<TkTaskDtl> selectByIds(List<String> ids, List<String> includeCols, List<String> excludeCols) {
        List<TkTaskDtl> objList = modelHelper.selectByIds(ids, includeCols, excludeCols);
        return objList;
    }
    ...
    public static List<TkTaskDtl> selectByWhere(Where where, List<String> includeCols, List<String> excludeCols) {
        List<TkTaskDtl> objList = modelHelper.selectByWhere(where, includeCols, excludeCols);
        return objList;
    }
