...
//        }
        ...
        // TODO 其他字段的处理逻辑。
        //写入初概流程业务表
        String error1 = insertInvest2(newImport,pmPrj,myJdbcTemplate);
        ...
        PmPrjExt.updatePrjBase(pmPrjId,prjMap,"PM_PRJ_INVEST2",3,myJdbcTemplate);
     ...
     * @return 资金信息map
     */
    private Map<String, Object> getInvestMap(FinancialImport newImport) {
     ...
     * @return 错误信息
     */
    private String insertInvest2(FinancialImport newImport, PmPrj pmPrj, MyJdbcTemplate myJdbcTemplate) {
        String projectId = newImport.getPmPrjId();
        String id = "";
        List<Map<String,Object>> list = myJdbcTemplate.queryForList("select id from PM_PRJ_INVEST2 where pm_prj_id = ? AND STATUS NOT IN ('VD','VDING')",projectId);
        if (CollectionUtils.isEmpty(list)){
            id = Crud.from("PM_PRJ_INVEST2").insertData();
        } else {
