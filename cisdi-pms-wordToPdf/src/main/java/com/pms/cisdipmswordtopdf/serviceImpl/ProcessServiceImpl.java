...
import com.pms.cisdipmswordtopdf.model.ProcessModel;
import com.pms.cisdipmswordtopdf.model.ProcessReq;
import com.pms.cisdipmswordtopdf.service.ProcessService;
import com.pms.cisdipmswordtopdf.strategy.MergeStrategy;
import com.pms.cisdipmswordtopdf.util.ExportUtil;
...
//        ExportUtil.exportExcel(response,"流程使用情况",processModels,heads);
    ...
    public List<ProcessModel> getProcessModels(ProcessReq processReq) {
        List<Map<String, Object>> processMaps = jdbcTemplate.queryForList("select p.name processName,IFNULL(p.num,0) processNum,IFNULL(ep.num,0) " +
                ...
                "left join (select a.WF_PROCESS_ID,b.name,count(*) num from wf_process_instance a left join wf_process b on a.WF_PROCESS_ID = b.id " +
                "WHERE START_DATETIME >= ? and b.STATUS in ('VDING','AP') and a.START_USER_ID != '0099250247095871681' and a" +
                ".status not in ('VD','VDING') GROUP BY a.WF_PROCESS_ID) sp on sp.WF_PROCESS_ID = p.WF_PROCESS_ID\n" +
                "left join (select a.WF_PROCESS_ID,b.name,count(*) num from wf_process_instance a left join wf_process b on a.WF_PROCESS_ID = b.id " +
                "WHERE a.START_DATETIME >= ? and a.END_DATETIME is not null and b.STATUS in ('VDING','AP') and a.START_USER_ID " +
                "!= '0099250247095871681' and a.status not in ('VD','VDING') GROUP BY a.WF_PROCESS_ID) sep on sep.WF_PROCESS_ID = p.WF_PROCESS_ID",
                processReq.getStartDate(),processReq.getStartDate());
    ...
    private String getOtherInfo(){
        List<Map<String, Object>> otherList = jdbcTemplate.queryForList("select count(*) num from wf_task where  CRT_DT <= now() and WF_TASK_TYPE_ID = " +
                "'TODO' AND AD_USER_ID != '0099250247095871681' union all\n" +
                "select count(*) from wf_task where  CRT_DT <= now() and WF_TASK_TYPE_ID = 'TODO' AND AD_USER_ID != '0099250247095871681' and " +
                "IS_CLOSED = 1 union all\n" +
                "select count(*) from wf_node_instance where CRT_DT <= now()");
    ...
    private List<List<String>> getHeads(ProcessReq processReq){
        ArrayList<String> headNames = new ArrayList<>(Arrays.asList("流程名称", "上线至今发起数", "上线至今结束数", processReq.getStartDate() + "至今发起数",
                processReq.getStartDate() + "至今结束数"));
