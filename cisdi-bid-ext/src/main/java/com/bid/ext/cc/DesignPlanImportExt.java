...
public class DesignPlanImportExt {
    ...
    public void designPlanImportFromExcel() {
        ...
        for (String fileId : fileIdList) {
            try {
                ...
                // 3. 读取Excel并构建树形结构
                List<CcPrjStructNode> nodes = readAndBuildTreeDesign(file);
    ...
    public void designPlanTemImportFromExcel() {
        ...
        for (String fileId : fileIdList) {
            try {
                ...
                // 3. 读取Excel并构建树形结构
                List<CcPrjStructNode> nodes = readAndBuildTreeDesignTem(file);
     ...
     * 读取Excel并构建树形结构
     */
    private List<CcPrjStructNode> readAndBuildTreeDesign(File file) throws IOException {
...
//            Object adUserId = Crud.from("CC_PRJ_MEMBER").where().eq(CcPrjMember.Cols.NAME, wbsChiefUserName).eq(CcPrjMember.Cols.IS_PRIMARY_POS, true).eq(CcPrjMember.Cols.CC_PRJ_ID, pCcPrjIds).select().specifyCols(CcPrjMember.Cols.AD_USER_ID).execForValue();
     ...
     * 读取Excel并构建树形结构
     */
    private List<CcPrjStructNode> readAndBuildTreeDesignTem(File file) throws IOException {
