...
public class WfNode {
    ...
    public static class Cols {
         ...
         * 进入时扩展。
         */
        public static final String IN_EXT_ID = "IN_EXT_ID";
        /**
         * 转办后扩展。
         */
        public static final String DISPATCHED_EXT_ID = "DISPATCHED_EXT_ID";
        /**
         * 能否跳过。
         */
         ...
         * 能否退回。
         */
        public static final String CAN_REJECT = "CAN_REJECT";
        /**
         * 能否撤回。
         */
         ...
         * 能否作废。
         */
        public static final String CAN_VOID = "CAN_VOID";
        /**
         * 能否结束。
         */
        public static final String CAN_END = "CAN_END";
        /**
         * 序号。
         */
     ...
     * 设置：进入时扩展。
     */
    public WfNode setInExtId(String inExtId) {
        if (this.inExtId == null && inExtId == null) {
            // 均为null，不做处理。
        } else if (this.inExtId != null && inExtId != null) {
            // 均非null，判定不等，再做处理：
            if (this.inExtId.compareTo(inExtId) != 0) {
                this.inExtId = inExtId;
                if (!this.toUpdateCols.contains("IN_EXT_ID")) {
                    this.toUpdateCols.add("IN_EXT_ID");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
     ...
     * 转办后扩展。
     */
    private String dispatchedExtId;

    /**
     * 获取：转办后扩展。
     */
    public String getDispatchedExtId() {
        return this.dispatchedExtId;
    }

    /**
     * 设置：转办后扩展。
     */
    public WfNode setDispatchedExtId(String dispatchedExtId) {
        if (this.dispatchedExtId == null && dispatchedExtId == null) {
            // 均为null，不做处理。
        } else if (this.dispatchedExtId != null && dispatchedExtId != null) {
            // 均非null，判定不等，再做处理：
            if (this.dispatchedExtId.compareTo(dispatchedExtId) != 0) {
                this.dispatchedExtId = dispatchedExtId;
                if (!this.toUpdateCols.contains("DISPATCHED_EXT_ID")) {
                    this.toUpdateCols.add("DISPATCHED_EXT_ID");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
     ...
     * 设置：能否退回。
     */
    public WfNode setCanReject(Boolean canReject) {
        if (this.canReject == null && canReject == null) {
            // 均为null，不做处理。
        } else if (this.canReject != null && canReject != null) {
            // 均非null，判定不等，再做处理：
            if (this.canReject.compareTo(canReject) != 0) {
                this.canReject = canReject;
                if (!this.toUpdateCols.contains("CAN_REJECT")) {
                    this.toUpdateCols.add("CAN_REJECT");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
     ...
     * 设置：能否撤回。
     */
    public WfNode setCanRevoke(Boolean canRevoke) {
        if (this.canRevoke == null && canRevoke == null) {
            // 均为null，不做处理。
        } else if (this.canRevoke != null && canRevoke != null) {
            // 均非null，判定不等，再做处理：
            if (this.canRevoke.compareTo(canRevoke) != 0) {
                this.canRevoke = canRevoke;
                if (!this.toUpdateCols.contains("CAN_REVOKE")) {
                    this.toUpdateCols.add("CAN_REVOKE");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
     ...
     * 设置：能否转办。
     */
    public WfNode setCanDispatch(Boolean canDispatch) {
     ...
     * 设置：能否作废。
     */
    public WfNode setCanVoid(Boolean canVoid) {
        if (this.canVoid == null && canVoid == null) {
            // 均为null，不做处理。
        } else if (this.canVoid != null && canVoid != null) {
            // 均非null，判定不等，再做处理：
            if (this.canVoid.compareTo(canVoid) != 0) {
                this.canVoid = canVoid;
                if (!this.toUpdateCols.contains("CAN_VOID")) {
                    this.toUpdateCols.add("CAN_VOID");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
     ...
     * 设置：能否结束。
     */
    public WfNode setCanEnd(Boolean canEnd) {
        if (this.canEnd == null && canEnd == null) {
            // 均为null，不做处理。
        } else if (this.canEnd != null && canEnd != null) {
            // 均非null，判定不等，再做处理：
            if (this.canEnd.compareTo(canEnd) != 0) {
                this.canEnd = canEnd;
                if (!this.toUpdateCols.contains("CAN_END")) {
                    this.toUpdateCols.add("CAN_END");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
     ...
     * 设置：额外信息。
     */
    public WfNode setExtraInfo(String extraInfo) {
     ...
     * @return 获取到的对象，若无则为null。
     */
    public static WfNode selectById(String id) {
     ...
     * @return 获取到的对象列表，若无则为null。建议使用CollectionUtils.isEmpty(list)方法判断有无。
     */
    public static List<WfNode> selectByIds(List<String> ids) {
     ...
     * @return 获取到的对象列表，若无则为null。建议使用CollectionUtils.isEmpty(list)方法判断有无。
     */
    public static List<WfNode> selectByWhere(Where where) {
     ...
     * @return 获取到的对象。
     */
    public static WfNode selectOneByWhere(Where where, List<String> includeCols, List<String> excludeCols) {
        List<WfNode> objList = modelHelper.selectByWhere(where, includeCols, excludeCols);
        if (objList != null && objList.size() > 1) {
            throw new BaseException("调用WfNode.selectOneByWhere方法不能返回" + objList.size() + "条记录（只能返回0条或1条）！");
        }
     ...
     * @return 获取到的对象。
     */
    public static WfNode selectOneByWhere(Where where) {
     ...
     * @param toModel   到模型。
     */
    public static void copyCols(WfNode fromModel, WfNode toModel) {
