...
public class AdAtt {
    ...
    public AdAtt setFileImgCompressSize(String fileImgCompressSize) {
        if (this.fileImgCompressSize == null && fileImgCompressSize == null) {
            // 均为null，不做处理。
        } else if (this.fileImgCompressSize != null && fileImgCompressSize != null) {
            // 均非null，判定不等，再做处理：
            if (this.fileImgCompressSize.compareTo(fileImgCompressSize) != 0) {
                this.fileImgCompressSize = fileImgCompressSize;
                if (!this.toUpdateCols.contains("FILE_IMG_COMPRESS_SIZE")) {
                    this.toUpdateCols.add("FILE_IMG_COMPRESS_SIZE");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
    ...
    public AdAtt setDefaultValueLogicFrom(String defaultValueLogicFrom) {
        if (this.defaultValueLogicFrom == null && defaultValueLogicFrom == null) {
            // 均为null，不做处理。
        } else if (this.defaultValueLogicFrom != null && defaultValueLogicFrom != null) {
            // 均非null，判定不等，再做处理：
            if (this.defaultValueLogicFrom.compareTo(defaultValueLogicFrom) != 0) {
                this.defaultValueLogicFrom = defaultValueLogicFrom;
                if (!this.toUpdateCols.contains("DEFAULT_VALUE_LOGIC_FROM")) {
                    this.toUpdateCols.add("DEFAULT_VALUE_LOGIC_FROM");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
    ...
    public AdAtt setDefaultValueLogicTo(String defaultValueLogicTo) {
        if (this.defaultValueLogicTo == null && defaultValueLogicTo == null) {
            // 均为null，不做处理。
        } else if (this.defaultValueLogicTo != null && defaultValueLogicTo != null) {
            // 均非null，判定不等，再做处理：
            if (this.defaultValueLogicTo.compareTo(defaultValueLogicTo) != 0) {
                this.defaultValueLogicTo = defaultValueLogicTo;
                if (!this.toUpdateCols.contains("DEFAULT_VALUE_LOGIC_TO")) {
                    this.toUpdateCols.add("DEFAULT_VALUE_LOGIC_TO");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
    ...
    public AdAtt setRefedDtlViewId(String refedDtlViewId) {
        if (this.refedDtlViewId == null && refedDtlViewId == null) {
            // 均为null，不做处理。
        } else if (this.refedDtlViewId != null && refedDtlViewId != null) {
            // 均非null，判定不等，再做处理：
            if (this.refedDtlViewId.compareTo(refedDtlViewId) != 0) {
                this.refedDtlViewId = refedDtlViewId;
                if (!this.toUpdateCols.contains("REFED_DTL_VIEW_ID")) {
                    this.toUpdateCols.add("REFED_DTL_VIEW_ID");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
    ...
    public AdAtt setRefedDtlViewEnabled(Boolean refedDtlViewEnabled) {
        if (this.refedDtlViewEnabled == null && refedDtlViewEnabled == null) {
            // 均为null，不做处理。
        } else if (this.refedDtlViewEnabled != null && refedDtlViewEnabled != null) {
            // 均非null，判定不等，再做处理：
            if (this.refedDtlViewEnabled.compareTo(refedDtlViewEnabled) != 0) {
                this.refedDtlViewEnabled = refedDtlViewEnabled;
                if (!this.toUpdateCols.contains("REFED_DTL_VIEW_ENABLED")) {
                    this.toUpdateCols.add("REFED_DTL_VIEW_ENABLED");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
    ...
    public AdAtt setIsDtlLink(Boolean isDtlLink) {
        if (this.isDtlLink == null && isDtlLink == null) {
            // 均为null，不做处理。
        } else if (this.isDtlLink != null && isDtlLink != null) {
            // 均非null，判定不等，再做处理：
            if (this.isDtlLink.compareTo(isDtlLink) != 0) {
                this.isDtlLink = isDtlLink;
                if (!this.toUpdateCols.contains("IS_DTL_LINK")) {
                    this.toUpdateCols.add("IS_DTL_LINK");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
    ...
    public AdAtt setHideInListOnXs(Boolean hideInListOnXs) {
        if (this.hideInListOnXs == null && hideInListOnXs == null) {
            // 均为null，不做处理。
        } else if (this.hideInListOnXs != null && hideInListOnXs != null) {
            // 均非null，判定不等，再做处理：
            if (this.hideInListOnXs.compareTo(hideInListOnXs) != 0) {
                this.hideInListOnXs = hideInListOnXs;
                if (!this.toUpdateCols.contains("HIDE_IN_LIST_ON_XS")) {
                    this.toUpdateCols.add("HIDE_IN_LIST_ON_XS");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
    ...
    public AdAtt setHideInListOnSm(Boolean hideInListOnSm) {
        if (this.hideInListOnSm == null && hideInListOnSm == null) {
            // 均为null，不做处理。
        } else if (this.hideInListOnSm != null && hideInListOnSm != null) {
            // 均非null，判定不等，再做处理：
            if (this.hideInListOnSm.compareTo(hideInListOnSm) != 0) {
                this.hideInListOnSm = hideInListOnSm;
                if (!this.toUpdateCols.contains("HIDE_IN_LIST_ON_SM")) {
                    this.toUpdateCols.add("HIDE_IN_LIST_ON_SM");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
    ...
    public AdAtt setHideInListOnMd(Boolean hideInListOnMd) {
        if (this.hideInListOnMd == null && hideInListOnMd == null) {
            // 均为null，不做处理。
        } else if (this.hideInListOnMd != null && hideInListOnMd != null) {
            // 均非null，判定不等，再做处理：
            if (this.hideInListOnMd.compareTo(hideInListOnMd) != 0) {
                this.hideInListOnMd = hideInListOnMd;
                if (!this.toUpdateCols.contains("HIDE_IN_LIST_ON_MD")) {
                    this.toUpdateCols.add("HIDE_IN_LIST_ON_MD");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
    ...
    public AdAtt setHideInListOnLg(Boolean hideInListOnLg) {
        if (this.hideInListOnLg == null && hideInListOnLg == null) {
            // 均为null，不做处理。
        } else if (this.hideInListOnLg != null && hideInListOnLg != null) {
            // 均非null，判定不等，再做处理：
            if (this.hideInListOnLg.compareTo(hideInListOnLg) != 0) {
                this.hideInListOnLg = hideInListOnLg;
                if (!this.toUpdateCols.contains("HIDE_IN_LIST_ON_LG")) {
                    this.toUpdateCols.add("HIDE_IN_LIST_ON_LG");
                }
            }
        } else {
            // 一者为null、一者非null，直接处理：
    ...
    public AdAtt setHideInListOnXl(Boolean hideInListOnXl) {
        if (this.hideInListOnXl == null && hideInListOnXl == null) {
            // 均为null，不做处理。
        } else if (this.hideInListOnXl != null && hideInListOnXl != null) {
            // 均非null，判定不等，再做处理：
            if (this.hideInListOnXl.compareTo(hideInListOnXl) != 0) {
     ...
     * @return 获取到的对象，若无则为null。
     */
    public static AdAtt selectById(String id) {
     ...
     * @return 获取到的对象列表，若无则为null。建议使用SharedUtil.isEmpty(list)方法判断有无。
     */
    public static List<AdAtt> selectByIds(List<String> ids) {
     ...
     * @return 获取到的对象列表，若无则为null。建议使用SharedUtil.isEmpty(list)方法判断有无。
     */
    public static List<AdAtt> selectByWhere(Where where) {
     ...
     * @return 获取到的对象。
     */
    public static AdAtt selectOneByWhere(Where where, List<String> includeCols, List<String> excludeCols) {
        List<AdAtt> objList = modelHelper.selectByWhere(where, includeCols, excludeCols);
        if (objList != null && objList.size() > 1) {
            throw new BaseException("调用AdAtt.selectOneByWhere方法不能返回" + objList.size() + "条记录（只能返回0条或1条）！");
        }
     ...
     * @return 获取到的对象。
     */
    public static AdAtt selectOneByWhere(Where where) {
     ...
     * @param toModel   到模型。
     */
    public static void copyCols(AdAtt fromModel, AdAtt toModel) {
