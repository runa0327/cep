...
public class QbqExt {
    ...
    public void getTaskList() {
        ...
        if(extApiParamMap.get("iSigned")!=null && (Boolean) extApiParamMap.get("iSigned")) {
            body.put("signerId", userId);
        }
        if(extApiParamMap.get("iPublished")!=null && (Boolean) extApiParamMap.get("iPublished")) {
    ...
    public void genChangeSignPdf() {
        ...
        for (EntityRecord entityRecord : entityRecordList) {
            signTaskCreate(entityRecord,year,month,day,loginInfo);
        }
    ...
    private  void   signTaskCreate(EntityRecord entityRecord ,int year,String month,String day,LoginInfo loginInfo){
        ...
        List<QbqBody.User> userList = queryQbqUser(wfProcessInstanceId,csCommId);
...
//            String path = flPath.getDir() + year + "/" + month + "/" + day + "/" + fileId + ".pdf";
        ...
        String path = "/Users/hejialun/Documents/new-ql/" + year + "/" + month + "/" + day + "/" + fileId + ".pdf";
...
//                flFile.setIsPublicRead(flPath.getIsPublicRead());
            ...
            initiateSigning( userList, fileId, wfProcessInstanceId, csCommId);
    ...
    private List<QbqBody.User> queryQbqUser(String wfProcessInstanceId,String csCommId){
        ...
        String queryUsers  =  "SELECT DISTINCT wt.AD_USER_ID,u.EXTRA_INFO,U.MOBILE,U.`NAME` USER_NAME from wf_task wt,AD_USER U ,ad_act aa where  WF_PROCESS_INSTANCE_ID=? AND   wt.ad_user_id=u.id AND  wt.ad_act_id=aa.id  AND  aa.`name`->>'$.ZH_CN' in ('提交','批准')";
...
//            String userName = adUser.getName();
     ...
     * 发起签署
     */
    private void initiateSigning( List<QbqBody.User> userList, String fileId, String wfProcessInstanceId, String csCommId) {
