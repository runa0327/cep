...
public class CcEngineeringQuantityServiceImpl implements CcEngineeringQuantityService {
     ...
     */
    @Override
    public void dealRocketMQData(List<EngineeringMetrics> sourceList) {
        if (!CollectionUtils.isEmpty(sourceList)){
            ...
            for (EngineeringMetrics tmp : sourceList) {
                //  单条数据处理并写入工程量填报
                dealAndInsertEngineeringQuantity(tmp,unitProjectList,now,createBy);
            }
        }
    }
     ...
     */
    private void dealAndInsertEngineeringQuantity(EngineeringMetrics tmp, List<CcPrjStructNode> unitProjectList, String now, String createBy) {
        List<CcEngineeringQuantity> list = new ArrayList<>();


        if (tmp.getPile() != null){ //  桩（m³）
            CcEngineeringQuantity ccEngineeringQuantity = normalNewCcEngineeringQuantity(now,createBy);
            ccEngineeringQuantity.setPrjStructNodeId(getPrjStructNodeIdByCode(tmp.getUnitProjectCode(),unitProjectList));
            ccEngineeringQuantity.setEngineeringQuantityTypeId("PILE");
            ccEngineeringQuantity.setUomTypeId("m³");
            //  此处缺少填报类型
            ccEngineeringQuantity.setTotalWeight(tmp.getPile());
            list.add(ccEngineeringQuantity);
        }
        if (tmp.getInfrastructure() != null){    //  基础砼（m³）
            CcEngineeringQuantity ccEngineeringQuantity = normalNewCcEngineeringQuantity(now,createBy);
            ccEngineeringQuantity.setPrjStructNodeId(getPrjStructNodeIdByCode(tmp.getUnitProjectCode(),unitProjectList));
            ccEngineeringQuantity.setEngineeringQuantityTypeId("FOUNDATION");
            ccEngineeringQuantity.setUomTypeId("m³");
            ccEngineeringQuantity.setTotalWeight(tmp.getInfrastructure());
            list.add(ccEngineeringQuantity);
        }
        if (tmp.getPipeline() != null){    //  管道（t）
            CcEngineeringQuantity ccEngineeringQuantity = normalNewCcEngineeringQuantity(now,createBy);
            ccEngineeringQuantity.setPrjStructNodeId(getPrjStructNodeIdByCode(tmp.getUnitProjectCode(),unitProjectList));
            ccEngineeringQuantity.setEngineeringQuantityTypeId("PIPELINE");
            ccEngineeringQuantity.setUomTypeId("t");
            ccEngineeringQuantity.setTotalWeight(tmp.getPipeline());
            list.add(ccEngineeringQuantity);
        }
        if (tmp.getQj() != null){    //  桥架（t）
            CcEngineeringQuantity ccEngineeringQuantity = normalNewCcEngineeringQuantity(now,createBy);
            ccEngineeringQuantity.setPrjStructNodeId(getPrjStructNodeIdByCode(tmp.getUnitProjectCode(),unitProjectList));
            ccEngineeringQuantity.setEngineeringQuantityTypeId("PIPELINE");
            ccEngineeringQuantity.setUomTypeId("t");
            ccEngineeringQuantity.setTotalWeight(tmp.getQj());
            list.add(ccEngineeringQuantity);
        }
        if (tmp.getSteelwork() != null){    //  钢结构（t）
            CcEngineeringQuantity ccEngineeringQuantity = normalNewCcEngineeringQuantity(now,createBy);
            ccEngineeringQuantity.setPrjStructNodeId(getPrjStructNodeIdByCode(tmp.getUnitProjectCode(),unitProjectList));
            ccEngineeringQuantity.setEngineeringQuantityTypeId("STEELSTRUCTURE");
            ccEngineeringQuantity.setUomTypeId("t");
            ccEngineeringQuantity.setTotalWeight(tmp.getSteelwork());
            list.add(ccEngineeringQuantity);
        }
        //  还缺少上部结构砼（m³）
